...
begin
  ...
  Canvas.Pen.Width := round(thickness); //* scale);         -Увеличение толщины при масштабировании
...
begin
  ...
  Vertices[High(Vertices)] := ScrToWorldCrds(X, Y);
  If ScrToWorldCrds(X, Y).X < bounds.Left then
    bounds.Left := ScrToWorldCrds(X, Y).X;
  If ScrToWorldCrds(X, Y).Y < bounds.Top then
    bounds.Top := ScrToWorldCrds(X, Y).Y;
  If ScrToWorldCrds(X, Y).X > bounds.Right then
    bounds.Right := ScrToWorldCrds(X, Y).X;
  If ScrToWorldCrds(X, Y).Y > bounds.Bottom then
    bounds.Bottom := ScrToWorldCrds(X, Y).Y;
end;

procedure TTwoPointFigure.AddPoint(X, Y: Integer; first: Boolean);
begin
  if first then
    bounds := DoubleRect(ScrToWorldCrds(X, Y), ScrToWorldCrds(X, Y))
  else
    bounds.BottomRight := ScrToWorldCrds(X, Y)
end;
